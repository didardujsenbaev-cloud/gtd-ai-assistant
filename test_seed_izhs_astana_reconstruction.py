"""
Tests for seed_izhs_astana_reconstruction.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_astana_reconstruction"
...
class TestCreatesService(unittest.TestCase):
    ...
    def test_2_service_id_constant(self):
        """2: SERVICE_ID == SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-AST-001")
    ...
    def test_2_service_category(self):
        """2: категория — private_house_reconstruction."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["service_category"], "private_house_reconstruction")

    def test_2_service_price_700k(self):
        """2: базовая стоимость от 700 000 тг."""
        seed = _fresh()
    ...
    def test_2_notes_no_seismic(self):
        """2: notes указывают что сейсмостойкость не нужна."""
        seed = _fresh()
        self.assertIn("Сейсмостойкое", seed.SERVICE_DATA["notes"])
...
# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-AST-RECON-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-AST-RECON-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-AST-RECON-001")

    def test_3_template_case_type(self):
        """3: case_type == astana_izhs_reconstruction_standard."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["case_type"], "astana_izhs_reconstruction_standard")

    def test_3_template_city_astana(self):
        """3: template city — Астана."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA.get("description", ""), seed.TEMPLATE_DATA["description"])
        self.assertIn("Астана", seed.TEMPLATE_DATA["template_name"])

    def test_3_template_links_to_service(self):
        """3: template привязан к SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-AST-001")

    def test_3_service_has_default_template(self):
        """3: SERVICE_DATA.default_roadmap_template_id == RMT-IZH-AST-RECON-001."""
        seed = _fresh()
        self.assertEqual(
            seed.SERVICE_DATA["default_roadmap_template_id"],
            "RMT-IZH-AST-RECON-001",
        )
...
class TestCreatesStages(unittest.TestCase):
    ...
    def test_4_stages_order_1_to_12(self):
        """4: порядковые номера 1-12."""
        seed   = _fresh()
    ...
    def test_4_has_apz_stage(self):
        """4: есть этап получения АПЗ."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("АПЗ" in n for n in names))

    def test_4_partial_stages_adds_missing(self):
        """4: если 5 этапов уже есть — добавляет только 7."""
        seed      = _fresh()
        add_calls = []
    ...
    def test_4_no_seismic_in_stages(self):
        """4: ни один этап не упоминает сейсмостойкость как обязательную."""
        seed = _fresh()
        for s in seed.STAGES:
            self.assertNotIn("Сейсмостойкое заключение", s.get("stage_name", ""))
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-AST-RECON-DOCS-001")
    ...
    def test_5_checklist_has_17_items(self):
        ...
        self.assertEqual(len(items), 17, f"Ожидалось 17 пунктов, нашли {len(items)}: {items}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-AST-RECON-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-AST-RECON-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-AST-001")

    def test_5_checklist_city_astana(self):
        """5: название чек-листа содержит Астан (Астана/Астане)."""
        seed = _fresh()
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-AST-RECON-PRIMARY-001")
    ...
    def test_6_sop_mentions_template(self):
        """6: SOP ссылается на RMT-IZH-AST-RECON-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-AST-RECON-001", seed.SOP_DATA["steps"])

    def test_6_sop_links_to_service(self):
        """6: SOP привязан к SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertEqual(seed.SOP_DATA["service_id"], "SVC-IZH-AST-001")

    def test_6_sop_city_astana(self):
        """6: SOP заголовок содержит Астан (Астана/Астане)."""
        seed = _fresh()
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_astana_reconstruction.py"
        )
    ...
    def test_9_astana_differs_from_almaty(self):
        """9: Астанинский seed отличается от Алматинского по city."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["city"], "Астана")
