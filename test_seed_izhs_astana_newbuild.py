"""
Tests for seed_izhs_astana_newbuild.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_astana_newbuild"
...
class TestCreatesService(unittest.TestCase):
    ...
    def test_2_service_id_constant(self):
        """2: SERVICE_ID == SVC-IZH-AST-002."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-AST-002")
    ...
    def test_2_service_category(self):
        """2: категория — private_house_new_construction."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["service_category"], "private_house_new_construction")
    ...
    def test_2_notes_no_seismic(self):
        ...
        seed = _fresh()
        self.assertIn("Сейсмостойкость", seed.SERVICE_DATA["notes"])
...
# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-AST-NEWBUILD-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-AST-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-AST-NEWBUILD-001")

    def test_3_template_case_type(self):
        """3: case_type == astana_izhs_newbuild_before_smr."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["case_type"], "astana_izhs_newbuild_before_smr")
    ...
    def test_3_template_links_to_service(self):
        """3: template привязан к SVC-IZH-AST-002."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-AST-002")

    def test_3_service_has_default_template(self):
        """3: SERVICE_DATA.default_roadmap_template_id == RMT-IZH-AST-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(
            seed.SERVICE_DATA["default_roadmap_template_id"],
            "RMT-IZH-AST-NEWBUILD-001",
        )
...
class TestCreatesStages(unittest.TestCase):
    ...
    def test_4_first_stage_bare_plot(self):
        """4: первый этап — анализ голого участка."""
        seed = _fresh()
        self.assertIn("голый", seed.STAGES[0]["stage_name"].lower())
    ...
    def test_4_has_smr_stage(self):
        """4: есть этап проведения СМР."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("СМР" in n for n in names))

    def test_4_partial_adds_missing(self):
        """4: если 6 этапов уже есть — добавляет только 6."""
        seed      = _fresh()
        add_calls = []
        ...
        with patch(f"{SEED_MODULE}._service_exists",   return_value=True), \
             patch(f"{SEED_MODULE}._template_exists",  return_value=True), \
             patch(f"{SEED_MODULE}._stages_count",     return_value=6), \
             patch(f"{SEED_MODULE}._checklist_exists", return_value=True), \
             patch(f"{SEED_MODULE}._sop_exists",       return_value=True), \
             patch("business_core.roadmap_template_manager.add_roadmap_template_stage",
                   side_effect=mock_add), \
        ...
        self.assertEqual(len(add_calls), 6)
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-AST-NEWBUILD-DOCS-001")

    def test_5_checklist_has_14_items(self):
        """5: чек-лист содержит 14 пунктов."""
        seed  = _fresh()
        items = [i.strip() for i in seed.CHECKLIST_DATA["items"].split(";") if i.strip()]
        self.assertEqual(len(items), 14, f"Ожидалось 14 пунктов, нашли {len(items)}: {items}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-AST-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-AST-NEWBUILD-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-AST-002."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-AST-002")
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-AST-NEWBUILD-PRIMARY-001")
    ...
    def test_6_sop_mentions_template(self):
        """6: SOP ссылается на RMT-IZH-AST-NEWBUILD-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-AST-NEWBUILD-001", seed.SOP_DATA["steps"])

    def test_6_sop_links_to_service(self):
        """6: SOP привязан к SVC-IZH-AST-002."""
        seed = _fresh()
        self.assertEqual(seed.SOP_DATA["service_id"], "SVC-IZH-AST-002")
...
class TestDefaultTemplate(unittest.TestCase):
    ...
    def test_8_ast002_not_ast001(self):
        """8: SERVICE_ID не совпадает с SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertNotEqual(seed.SERVICE_ID, "SVC-IZH-AST-001")
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-AST-002")
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_astana_newbuild.py"
        )
    ...
    def test_9_notes_no_seismic_required(self):
        """9: Notes услуги указывают что сейсмостойкость не требуется."""
        seed = _fresh()
        self.assertIn("не требуется", seed.SERVICE_DATA["notes"].lower())
