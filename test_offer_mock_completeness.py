"""
Phase 40C (Commercial Offer Domain Foundation, ADR-023): PRS-003-class
regression guard for the Commercial Offer test surface — mirrors
test_payment_mock_completeness.py's own approach. Static, source-level
guard that makes a missing hard-socket-block registration structurally
...
class TestAllOfferTestFilesAreHardSocketBlocked(unittest.TestCase):
    ...
    def test_offer_domain_frozenset_included_in_hard_block(self):
        ...
        self.assertIn("_OFFER_DOMAIN_TEST_FILES", union_body)


class TestOfferManagerHasNoDirectNetworkAccess(unittest.TestCase):

    def test_offer_manager_only_depends_on_sheets(self):
    ...
    def test_offer_manager_never_constructs_its_own_sheets_client(self):
        """Writes are only safe because every Sheets object is obtained
        via business_core.sheets.get_business_sheet() — itself fully
        mockable. What must never happen is offer_manager.py
        ...
        self.assertEqual(hits, [], f"offer_manager.py bypassing get_business_sheet() with its own client: {hits}")

    def test_offer_manager_reads_sheets_only_through_get_business_sheet(self):
        path = BUSINESS_CORE / "offer_manager.py"
        src = path.read_text(encoding="utf-8")
        if "update_cell(" in src or "append_row(" in src or "get_all_values(" in src:
            self.assertIn("get_business_sheet", src, "offer_manager.py performs a Sheets operation without ever calling get_business_sheet()")
