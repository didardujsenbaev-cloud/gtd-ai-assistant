"""
Tests for seed_izhs_almaty_outbuilding.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_almaty_outbuilding"
...
class TestDryRun(unittest.TestCase):
    ...
    def test_1_dry_run_all_exist(self):
        ...
        with patch(f"{SEED_MODULE}._service_exists",   return_value=True), \
             ...
             patch(f"{SEED_MODULE}._stages_count",     return_value=19), \
    ...
    def test_1_dry_run_shows_5_creates(self):
        ...
        self.assertTrue(all("CREATE" in p for p in result["plan"]))


# ────────────────────────────────────────────────────────────
# 2. seed создает service SVC-IZH-003
# ────────────────────────────────────────────────────────────

class TestCreatesService(unittest.TestCase):

    def test_2_creates_service(self):
        """2: run_seed создает SVC-IZH-003."""
        seed  = _fresh()
    ...
    def test_2_service_id_is_svc_izh_003(self):
        """2: SERVICE_ID == SVC-IZH-003."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-003")

    def test_2_service_name(self):
        """2: Название услуги содержит хозпостройку."""
        seed = _fresh()
        self.assertIn("хозпостройк", seed.SERVICE_DATA["service_name"])

    def test_2_service_category(self):
        """2: категория услуги outbuilding_construction."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["service_category"], "outbuilding_construction")

    def test_2_service_different_from_001_and_002(self):
        """2: SVC-IZH-003 не равен SVC-IZH-001 и SVC-IZH-002."""
        seed = _fresh()
        self.assertNotEqual(seed.SERVICE_ID, "SVC-IZH-001")
        self.assertNotEqual(seed.SERVICE_ID, "SVC-IZH-002")
    ...
    def test_2_service_price_400k(self):
        """2: базовая стоимость от 400 000 тг."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["price_from"], "400000")


# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-ALM-OUTBUILDING-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-ALM-OUTBUILDING-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-ALM-OUTBUILDING-001")
    ...
    def test_3_template_case_type(self):
        ...
        self.assertEqual(
            ...
            "almaty_izhs_outbuilding_before_smr",
        )

    def test_3_template_links_to_svc_003(self):
        """3: template привязан к SVC-IZH-003."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-003")

    def test_3_service_has_default_template_id(self):
        """3: SERVICE_DATA содержит default_roadmap_template_id."""
        seed = _fresh()
        self.assertEqual(
            seed.SERVICE_DATA["default_roadmap_template_id"],
            "RMT-IZH-ALM-OUTBUILDING-001",
        )


# ────────────────────────────────────────────────────────────
# 4. seed создает 19 template stages
# ────────────────────────────────────────────────────────────

class TestCreatesStages(unittest.TestCase):

    def test_4_creates_19_stages(self):
        """4: run_seed создает 19 этапов."""
        seed      = _fresh()
        add_calls = []
        ...
        self.assertEqual(len(add_calls), 19)

    def test_4_stage_count_is_19(self):
        """4: в STAGES ровно 19 записей."""
        seed = _fresh()
        self.assertEqual(len(seed.STAGES), 19)

    def test_4_stages_order_1_to_19(self):
        """4: порядковые номера 1-19."""
        seed   = _fresh()
        orders = [s["order"] for s in seed.STAGES]
        self.assertEqual(orders, list(range(1, 20)))

    def test_4_first_stage_name(self):
        """4: первый этап — первичный анализ (пристроено или отдельно)."""
        seed = _fresh()
    ...
    def test_4_last_stage_name(self):
        ...
        seed = _fresh()
        self.assertIn("НАО", seed.STAGES[18]["stage_name"])

    def test_4_has_ispolnitelnaya_semka(self):
        """4: есть этап исполнительной съемки."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("Исполнительная съемка" in n for n in names))

    def test_4_has_eskizny_proekt(self):
        """4: есть этап эскизного проекта хозпостройки."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("Эскизный проект" in n for n in names))

    def test_4_has_existing_house_check_stage(self):
        """4: есть этап проверки существующего дома."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("зарегистрированного дома" in n for n in names))

    def test_4_partial_stages_adds_missing(self):
        """4: если 10 этапов уже есть — добавляет только 9."""
        seed      = _fresh()
        add_calls = []
        ...
        with patch(f"{SEED_MODULE}._service_exists",   return_value=True), \
             patch(f"{SEED_MODULE}._template_exists",  return_value=True), \
             patch(f"{SEED_MODULE}._stages_count",     return_value=10), \
             patch(f"{SEED_MODULE}._checklist_exists", return_value=True), \
             patch(f"{SEED_MODULE}._sop_exists",       return_value=True), \
             patch("business_core.roadmap_template_manager.add_roadmap_template_stage",
                   side_effect=mock_add), \
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-ALM-OUTBUILDING-DOCS-001")

    def test_5_checklist_has_20_items(self):
        """5: чек-лист содержит 20 пунктов."""
        seed  = _fresh()
        items = [i.strip() for i in seed.CHECKLIST_DATA["items"].split(";") if i.strip()]
        self.assertEqual(len(items), 20, f"Ожидалось 20 пунктов, нашли {len(items)}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-ALM-OUTBUILDING-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-ALM-OUTBUILDING-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-003."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-003")
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-ALM-OUTBUILDING-PRIMARY-001")
    ...
    def test_6_sop_mentions_outbuilding_template(self):
        """6: SOP ссылается на RMT-IZH-ALM-OUTBUILDING-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-ALM-OUTBUILDING-001", seed.SOP_DATA["steps"])

    def test_6_sop_redirects_to_svc_001(self):
        """6: SOP направляет к SVC-IZH-001 если пристроена к дому."""
        seed = _fresh()
        self.assertIn("SVC-IZH-001", seed.SOP_DATA["steps"])

    def test_6_sop_redirects_to_svc_002(self):
        """6: SOP направляет к SVC-IZH-002 если голый участок."""
        seed = _fresh()
        self.assertIn("SVC-IZH-002", seed.SOP_DATA["steps"])
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_almaty_outbuilding.py"
        )
    ...
    def test_9_notes_reference_svc_001(self):
        """9: Notes услуги ссылаются на SVC-IZH-001 (контроль разграничения)."""
        seed = _fresh()
        self.assertIn("SVC-IZH-001", seed.SERVICE_DATA["notes"])

    def test_9_template_notes_reference_svc_002(self):
        """9: Notes шаблона ссылаются на SVC-IZH-002."""
        seed = _fresh()
        self.assertIn("SVC-IZH-002", seed.TEMPLATE_DATA["notes"])
...
class TestEnvNotChanged(unittest.TestCase):
    ...
    def test_10_biz_id_constant(self):
        """10: BIZ_ID == BIZ-001."""
        seed = _fresh()
