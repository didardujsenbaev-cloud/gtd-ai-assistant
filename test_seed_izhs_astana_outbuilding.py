"""
Tests for seed_izhs_astana_outbuilding.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_astana_outbuilding"
...
class TestCreatesService(unittest.TestCase):
    ...
    def test_2_service_id_constant(self):
        """2: SERVICE_ID == SVC-IZH-AST-003."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-AST-003")
    ...
    def test_2_service_category(self):
        """2: категория — outbuilding_construction."""
        seed = _fresh()
    ...
    def test_2_notes_outbuilding_only(self):
        """2: notes указывают что услуга только для отдельно стоящей постройки."""
        seed = _fresh()
        self.assertIn("отдельно стоящей", seed.SERVICE_DATA["notes"])
...
# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-AST-OUTBUILDING-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-AST-OUTBUILDING-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-AST-OUTBUILDING-001")

    def test_3_template_case_type(self):
        """3: case_type == astana_izhs_outbuilding_before_smr."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["case_type"], "astana_izhs_outbuilding_before_smr")
    ...
    def test_3_template_links_to_service(self):
        """3: template привязан к SVC-IZH-AST-003."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-AST-003")

    def test_3_service_has_default_template(self):
        """3: SERVICE_DATA.default_roadmap_template_id == RMT-IZH-AST-OUTBUILDING-001."""
        seed = _fresh()
        self.assertEqual(
            seed.SERVICE_DATA["default_roadmap_template_id"],
            "RMT-IZH-AST-OUTBUILDING-001",
        )
...
class TestCreatesStages(unittest.TestCase):
    ...
    def test_4_first_stage_attached_or_standalone(self):
        """4: первый этап — анализ: пристроено или отдельно."""
        seed = _fresh()
        self.assertIn("пристроено", seed.STAGES[0]["stage_name"].lower())

    def test_4_last_stage_nao(self):
        """4: последний этап — акт ввода и регистрация в НАО."""
        seed = _fresh()
    ...
    def test_4_partial_adds_missing(self):
        """4: если 5 этапов уже есть — добавляет только 8."""
        seed      = _fresh()
        add_calls = []
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-AST-OUTBUILDING-DOCS-001")
    ...
    def test_5_checklist_has_16_items(self):
        ...
        self.assertEqual(len(items), 16, f"Ожидалось 16 пунктов, нашли {len(items)}: {items}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-AST-OUTBUILDING-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-AST-OUTBUILDING-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-AST-003."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-AST-003")
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-AST-OUTBUILDING-PRIMARY-001")
    ...
    def test_6_sop_mentions_template(self):
        """6: SOP ссылается на RMT-IZH-AST-OUTBUILDING-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-AST-OUTBUILDING-001", seed.SOP_DATA["steps"])

    def test_6_sop_links_to_service(self):
        """6: SOP привязан к SVC-IZH-AST-003."""
        seed = _fresh()
        self.assertEqual(seed.SOP_DATA["service_id"], "SVC-IZH-AST-003")
...
class TestDefaultTemplate(unittest.TestCase):
    ...
    def test_8_ast003_different_from_ast001_ast002(self):
        """8: SERVICE_ID уникальный — SVC-IZH-AST-003."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-AST-003")
        self.assertNotIn(seed.SERVICE_ID, ["SVC-IZH-AST-001", "SVC-IZH-AST-002"])
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_astana_outbuilding.py"
        )
    ...
    def test_9_notes_redirect_attached_to_ast001(self):
        """9: Notes указывают перенаправить пристройку в SVC-IZH-AST-001."""
        seed = _fresh()
        self.assertIn("SVC-IZH-AST-001", seed.SERVICE_DATA["notes"])
