"""
Tests for seed_izhs_almaty_newbuild.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_almaty_newbuild"
...
class TestDryRun(unittest.TestCase):
    ...
    def test_1_dry_run_all_exist(self):
        ...
        with patch(f"{SEED_MODULE}._service_exists",   return_value=True), \
             ...
             patch(f"{SEED_MODULE}._stages_count",     return_value=18), \
...
class TestCreatesService(unittest.TestCase):
    ...
    def test_2_service_id_is_svc_izh_002(self):
        """2: SERVICE_ID == SVC-IZH-002."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-002")

    def test_2_service_name(self):
        """2: Название услуги корректное."""
        seed = _fresh()
        self.assertIn("Новое строительство", seed.SERVICE_DATA["service_name"])

    def test_2_service_category(self):
        """2: категория услуги new_construction."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["service_category"], "new_construction")

    def test_2_service_different_from_svc_001(self):
        """2: SVC-IZH-002 отличается от SVC-IZH-001."""
        seed = _fresh()
...
# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-ALM-NEWBUILD-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-ALM-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-ALM-NEWBUILD-001")
    ...
    def test_3_template_case_type(self):
        ...
        self.assertEqual(
            ...
            "almaty_izhs_newbuild_before_smr",
        )

    def test_3_template_links_to_svc_002(self):
        """3: template привязан к SVC-IZH-002."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-002")
    ...
    def test_3_service_has_default_template_id(self):
        ...
        self.assertEqual(
            ...
            "RMT-IZH-ALM-NEWBUILD-001",
        )


# ────────────────────────────────────────────────────────────
# 4. seed создает 18 template stages
# ────────────────────────────────────────────────────────────

class TestCreatesStages(unittest.TestCase):

    def test_4_creates_18_stages(self):
        """4: run_seed создает 18 этапов."""
        seed      = _fresh()
        add_calls = []
        ...
        self.assertEqual(len(add_calls), 18)

    def test_4_stage_count_is_18(self):
        """4: в STAGES ровно 18 записей."""
        seed = _fresh()
        self.assertEqual(len(seed.STAGES), 18)

    def test_4_stages_order_1_to_18(self):
        """4: порядковые номера 1-18."""
        seed   = _fresh()
        orders = [s["order"] for s in seed.STAGES]
        self.assertEqual(orders, list(range(1, 19)))
    ...
    def test_4_last_stage_name(self):
        ...
        seed = _fresh()
        self.assertIn("НАО", seed.STAGES[17]["stage_name"])

    def test_4_has_ispolnitelnaya_semka_stage(self):
        """4: есть этап исполнительной съемки (характерен для нового строительства)."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("Исполнительная съемка" in n for n in names))

    def test_4_has_eskizny_proekt_stage(self):
        """4: есть этап эскизного проекта."""
        seed  = _fresh()
        names = [s["stage_name"] for s in seed.STAGES]
        self.assertTrue(any("Эскизный проект" in n for n in names))

    def test_4_partial_stages_adds_missing(self):
        """4: если 10 этапов уже есть — добавляет только недостающие 8."""
        seed      = _fresh()
        add_calls = []
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-ALM-NEWBUILD-DOCS-001")

    def test_5_checklist_has_17_items(self):
        """5: чек-лист содержит 17 пунктов."""
        seed  = _fresh()
        items = [i.strip() for i in seed.CHECKLIST_DATA["items"].split(";") if i.strip()]
        self.assertEqual(len(items), 17, f"Ожидалось 17 пунктов, нашли {len(items)}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-ALM-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-ALM-NEWBUILD-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-002."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-002")
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-ALM-NEWBUILD-PRIMARY-001")
    ...
    def test_6_sop_mentions_newbuild_template(self):
        """6: SOP ссылается на RMT-IZH-ALM-NEWBUILD-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-ALM-NEWBUILD-001", seed.SOP_DATA["steps"])

    def test_6_sop_mentions_svc_003_redirect(self):
        """6: SOP упоминает направление в SVC-IZH-003 при наличии дома."""
        seed = _fresh()
        self.assertIn("SVC-IZH-003", seed.SOP_DATA["steps"])
...
class TestDefaultTemplate(unittest.TestCase):
    ...
    def test_8_service_data_has_default_template(self):
        """8: SERVICE_DATA.default_roadmap_template_id = RMT-IZH-ALM-NEWBUILD-001."""
        seed = _fresh()
        self.assertEqual(
    ...
    def test_8_template_biz_matches_service_biz(self):
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_almaty_newbuild.py"
        )
    ...
    def test_9_service_notes_reference_svc_003(self):
        """9: Notes услуги ссылаются на SVC-IZH-003 (контроль разграничения)."""
        seed = _fresh()
        self.assertIn("SVC-IZH-003", seed.SERVICE_DATA["notes"])
...
class TestEnvNotChanged(unittest.TestCase):
    ...
    def test_10_price_from_correct(self):
        """10: базовая стоимость от 700 000 тг."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_DATA["price_from"], "700000")
