...
ВНИМАНИЕ: этот файл содержит LIVE-интеграционные тесты, которые
обращаются к реальному Google Sheets BUSINESS_CORE, а Тест 10
СОЗДАЁТ реальную тестовую строку BIZ-TEST в production BIZ_REGISTRY.

Безопасность (Phase 11F.2 — после случайной записи в Phase 11F.1):
  - Импорт этого модуля (в т.ч. через unittest discovery) НИЧЕГО
    не выполняет — вся логика находится под `if __name__ == "__main__"`.
  - Live-чтение (Тесты 6-9, 11-12) требует BUSINESS_CORE_ALLOW_LIVE_TESTS=1.
  - Live-запись (Тест 10, append_business_row → BIZ_REGISTRY)
    дополнительно требует BUSINESS_CORE_ALLOW_LIVE_WRITES=1.
  - Без флагов — тест сразу завершается (SKIPPED), без сети и записи.

Запуск (безопасный, без live):
    python3 test_business_core_sheets.py

Запуск с live-чтением:
    BUSINESS_CORE_ALLOW_LIVE_TESTS=1 python3 test_business_core_sheets.py

Запуск с live-чтением и live-записью (создаст тестовую строку BIZ-TEST):
    BUSINESS_CORE_ALLOW_LIVE_TESTS=1 BUSINESS_CORE_ALLOW_LIVE_WRITES=1 \\
        python3 test_business_core_sheets.py
"""
...
# ─────────────────────────────────────────────────────────────
# Phase 11F.2: явные флаги для live Sheets доступа
# ─────────────────────────────────────────────────────────────

ALLOW_LIVE_TESTS  = os.environ.get("BUSINESS_CORE_ALLOW_LIVE_TESTS")  == "1"
ALLOW_LIVE_WRITES = os.environ.get("BUSINESS_CORE_ALLOW_LIVE_WRITES") == "1"
...
def print_summary():
    ...
    if FAILED == 0:
        ...
        from business_core.sheets import get_spreadsheet_url
...
def _run_live_tests():
    """Вся live-логика теста business_core/sheets.py (Фаза 2A) —
    вызывается ТОЛЬКО из __main__ и только если
    BUSINESS_CORE_ALLOW_LIVE_TESTS=1 (Phase 11F.2). Тест 10
    (запись в production BIZ_REGISTRY) дополнительно требует
    BUSINESS_CORE_ALLOW_LIVE_WRITES=1."""
    ...
    if not cfg["ok"]:
        print("  ⏭ Пропущено")
    elif not ALLOW_LIVE_WRITES:
        print("  ⏭ Пропущено — требуется BUSINESS_CORE_ALLOW_LIVE_WRITES=1 "
              "(Phase 11F.2: запись в production Sheets заблокирована по умолчанию)")
    else:
...
if __name__ == "__main__":
    if not ALLOW_LIVE_TESTS:
        print("⏭  SKIPPED: test_business_core_sheets.py — live Google Sheets")
        print("    интеграционные тесты (Phase 11F.2 safety gate).")
        print("    Для запуска чтения:  BUSINESS_CORE_ALLOW_LIVE_TESTS=1 python3 test_business_core_sheets.py")
        print("    Для запуска записи:  дополнительно BUSINESS_CORE_ALLOW_LIVE_WRITES=1")
        sys.exit(0)
    _run_live_tests()
