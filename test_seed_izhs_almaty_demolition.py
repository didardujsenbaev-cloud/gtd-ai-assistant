"""
Tests for seed_izhs_almaty_demolition.py
...
SEED_MODULE   = "business_core.seeds.seed_izhs_almaty_demolition"
...
class TestCreatesService(unittest.TestCase):
    ...
    def test_2_service_id_is_svc_izh_004(self):
        """2: SERVICE_ID == SVC-IZH-004."""
        seed = _fresh()
        self.assertEqual(seed.SERVICE_ID, "SVC-IZH-004")

    def test_2_service_category_demolition(self):
        """2: категория услуги demolition."""
        seed = _fresh()
    ...
    def test_2_service_name_contains_snos(self):
        """2: название услуги содержит 'Снос'."""
        seed = _fresh()
        self.assertIn("Снос", seed.SERVICE_DATA["service_name"])
...
# ────────────────────────────────────────────────────────────
# 3. seed создает roadmap template RMT-IZH-ALM-DEMOLITION-001
# ────────────────────────────────────────────────────────────
...
class TestCreatesTemplate(unittest.TestCase):
    ...
    def test_3_template_id_constant(self):
        """3: TEMPLATE_ID == RMT-IZH-ALM-DEMOLITION-001."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_ID, "RMT-IZH-ALM-DEMOLITION-001")

    def test_3_template_case_type(self):
        """3: case_type == almaty_izhs_demolition."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["case_type"], "almaty_izhs_demolition")

    def test_3_template_links_to_svc_004(self):
        """3: template привязан к SVC-IZH-004."""
        seed = _fresh()
        self.assertEqual(seed.TEMPLATE_DATA["service_id"], "SVC-IZH-004")
...
class TestCreatesStages(unittest.TestCase):
    ...
    def test_4_first_stage_name(self):
        """4: первый этап — первичный анализ объекта на снос."""
        seed = _fresh()
        self.assertIn("Первичный анализ", seed.STAGES[0]["stage_name"])

    def test_4_last_stage_name(self):
        """4: последний этап — регистрация акта сноса."""
        seed = _fresh()
        self.assertIn("Регистрация акта сноса", seed.STAGES[8]["stage_name"])

    def test_4_has_demolition_stage(self):
...
class TestCreatesChecklist(unittest.TestCase):
    ...
    def test_5_checklist_id_constant(self):
        ...
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_ID, "CHK-IZH-ALM-DEMOLITION-DOCS-001")
    ...
    def test_5_checklist_has_12_items(self):
        ...
        self.assertEqual(len(items), 12, f"Ожидалось 12 пунктов, нашли {len(items)}")

    def test_5_checklist_links_to_template(self):
        """5: checklist привязан к RMT-IZH-ALM-DEMOLITION-001."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["template_id"], "RMT-IZH-ALM-DEMOLITION-001")

    def test_5_checklist_links_to_service(self):
        """5: checklist привязан к SVC-IZH-004."""
        seed = _fresh()
        self.assertEqual(seed.CHECKLIST_DATA["service_id"], "SVC-IZH-004")
...
class TestCreatesSOP(unittest.TestCase):
    ...
    def test_6_sop_id_constant(self):
        """6: SOP_ID корректный."""
        seed = _fresh()
        self.assertEqual(seed.SOP_ID, "SOP-IZH-ALM-DEMOLITION-PRIMARY-001")
    ...
    def test_6_sop_mentions_demolition_template(self):
        """6: SOP ссылается на RMT-IZH-ALM-DEMOLITION-001."""
        seed = _fresh()
        self.assertIn("RMT-IZH-ALM-DEMOLITION-001", seed.SOP_DATA["expected_result"])

    def test_6_sop_links_to_service(self):
        """6: SOP привязан к SVC-IZH-004."""
        seed = _fresh()
        self.assertEqual(seed.SOP_DATA["service_id"], "SVC-IZH-004")
...
class TestGTDIsolation(unittest.TestCase):
    ...
    def test_9_seed_file(self):
        ...
        self._check_file(
            WORKSPACE / "business_core" / "seeds" / "seed_izhs_almaty_demolition.py"
        )
    ...
    def test_9_notes_clarify_scope(self):
        """9: Notes услуги уточняют, что физический снос не входит."""
        seed = _fresh()
        self.assertIn("физический", seed.SERVICE_DATA["notes"].lower())
